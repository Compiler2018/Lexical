...
public class analyse {
    ...
    public List<Map> GetStringAndSortNum(String[] list) throws Exception {
        char firstChar;//用于记录第一个首字符
        String keyword="",sortclass="";//需要输出返回的关键字，种别
   ...
   */
            if(cType=="digit"){
                if(IsNum(word)){
                    keyword=word;
                    sortclass=s.getSortclass('n');
                    map.put("keyword", keyword);
                    map.put("sortclass", sortclass);
                    mList.add(map);
                }
                else{
            ...
            if(cType=="opts"){
                ...
                if(len==1){
                    if(IsSingleOpt(word)){
                        keyword=word;
                        sortclass=s.getSortclass(word.charAt(0));
                        map.put("keyword", keyword);
                        map.put("sortclass", sortclass);
                        mList.add(map);
                    }
 ...
 */
      ...
      /**      if(keyword.equals("")||sortclass.equals("")||keyword==""||sortclass=="")
                continue;
            else{
                map.put("keyword", keyword);
                map.put("sortclass", sortclass);
        ...
        switch(firstChar){
            case 'b':
                if(word=="begin"||word.equals("begin")){
                    String sortclass1=s.getSortclass(word)+"";
                    m.put("sortclass",sortclass1);
                    m.put("keyword", "begin");
                }
                break;
